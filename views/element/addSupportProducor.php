...
<div id="formSendMailInvite" class="hidden">
	...
	<div id="addMembers" style="line-height:40px; padding:0px;" autocomplete="off" submit='false'>
		<input type="hidden" id="parentOrganisation" name="parentId" value="<?php echo (string)$parentId; ?>"/>
	    <input type="hidden" id="memberId" name="memberId" value=""/>
	    <input type="hidden" id="linksType" name="linksType" value=""/>
...
var newMemberInCommunity = true;
var reloadOnClose=false;
var contactTypes = [{ name : "organizations", color: "green", icon:"group", label:"Organisations" }];
...
var addLinkSearchMode = "all";
jQuery(document).ready(function() {
	$(".btn-menu-element-prodsupport").click(function(){
		connectType=$(this).data("connect");
	});
	if(connectType=="producors")
		labelType="producteurs";
	else
		labelType="soutiens";
...
function buildModal(fieldObj, idUi){
				  ...
				  '<div class="modal-dialog">'+
				    '<div class="modal-content">'+
				      '<div class="modal-header">'+
						...
						'<h4 class="modal-title" id="myModalLabel"><i class="fa fa-search"></i> '+fieldObj.title1+' '+labelType+'</h4>'+
				      '</div>'+
	...
	fieldHTML +=							'</ul>' +
								...
								'</div>' +
					      	'</div>'+*/
					      	'<div class="no-padding pull-right col-md-12 col-sm-12 col-xs-12 bg-white" id="list-scroll-type">';
	fieldHTML += 			'</div>' +
						'</div>'+
					  '</div>'+
				      '<div class="modal-footer">'+
				      	...
				      	'<button id="btn-save" type="button" class="btn btn-success btn-sm" data-dismiss="modal"><i class="fa fa-check"></i> '+fieldObj.btnSaveTitle+' '+labelType+'</button>'+
				      '</div>'+
				    '</div><!-- /.modal-content -->'+
				  '</div><!-- /.modal-dialog -->';
								...
								'<div class="panel-body no-padding">'+
									'<div class="list-group padding-5">'+
										...
										if(typeof value != "undefined"){
											...
											var profilThumbImageUrl = (typeof value.profilThumbImageUrl != "undefined" && value.profilThumbImageUrl != "") ? baseUrl + value.profilThumbImageUrl : assetPath + "/images/thumb/default_organizations.png";
        	...
        	if(!data){
        		...
        		if(!notEmpty(data.citoyens) && !notEmpty(data.organizations)){
        			...
        			//$("input[name='memberType']").click(function(){
        				$("#fa-type-contact-mail").addClass("fa-group");
...
function sendInvitation(){
	...
	$.each($("#list-scroll-type input:checkbox"), function(key, val){
		if($(this).prop("checked") == true){
			...
			contactPublicFound = listContact;
	...
	$.ajax({
        type: "POST",
        url: baseUrl+"/communecter/poi/addlink",
        data: params,
        dataType: "json",
        success: function(data){
        	if(!data.result){
...
function sendInvitationMailAddMember(){ mylog.log("sendInvitationMailAddMember");
	...
	var params = {
		...
		"childType" : "<?php echo Organization::COLLECTION ?>", 
		"connectType" : connectType,
		"organizationType" : $("#addMembers #organizationType").val(),
