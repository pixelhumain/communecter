...
if( !isset($hideTexts) )
{
	if( isset( $action["urls"]) && count( $action["urls"] )  ){
		echo "Url(s) : <br/>";
		foreach ($action["urls"] as $u) {
...
<div class="<?php echo ( @$position ) ? $position : "pull-left"; ?> margin-10">
	<?php
		...
		$voteLinksAndInfos = Action::voteLinksAndInfos( true , $action );
...
if(isset( Yii::app()->session["userId"]) )
{
	...
	{
		if(!(isset($action[Action::ACTION_FOLLOW]) 
		    && is_array($action[Action::ACTION_FOLLOW]) 
		    && in_array(Yii::app()->session["userId"], $action[Action::ACTION_FOLLOW]))) 
		{
?>
		<br/>
		<?php /* ?>
		<a class="btn btn-xs btn-default" href="#" onclick="addaction('<?php echo (string)$action["_id"]?>','<?php echo Action::ACTION_FOLLOW ?>')">
	...
	<?php } 
		...
		<a class="btn btn-xs btn-default" onclick="entryDetail('<?php echo Yii::app()->createUrl("/communecter/survey/entry/id/".(string)$action["_id"])?>','edit')" href="javascript:;">
...
<br/>Views : <?php echo @$action["viewCount"]; ?>
*/?>
