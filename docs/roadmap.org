...
** @Bouboule 
  ...
  *** Notifications
  [X] Create notificationTree
  [X] Simplification of method with $construct
  [ ] Add method to get intelligent Notifications
    [X] Simple action on same notification with different author
    [ ] Limit of time (1 day Or two day?)
    [X] Update community to notify if one is out of element
  [ ] Commons method called to construct notification
    [X] /Follow/ => TEST OK
    [X] /Ask/ to become member or admin => TEST OK
    [X] /Accept/ an admin or member (include new sublevel in notficationTree) => TEST OK
    [X] /Invite/ => TEST OK
      [X] Generalize invitation as event for organization and project
    [X] /Confirm/ an invitation to join a element => TEST OK
    [X] /Join/ for creator of event, member of orga or project => TEST OK
    [X] /Like/Unlike/Comment => TEST OK ++
    [X] /Add/ => TEST OK 
    [X] /Post/ => TEST OK ++
    [X] /Vote/ => TEST OK
  ...
  [ ] Update or delete notification
    [o] Ask to Become => remove object if empty delete notification side
    [o] Confirm to become => delete notification on user side
  [X] Community to notify
    [X] method
    *** WishList
    [ ] City 
      [ ] Creator && community participating && community subscribing to theme or citizen council
  [ ] Create model notificationTree.php
  [ ] View of notification
    [X] Get new model of notification
    [o] Create $or in condition to get old notification
    [X] Plug notifification time on updated
    [o] Use unSee && unRead label in notifcation
    [X] Bug on lbh load
    *** Wishlist
    [ ] Separate invitation validation && become friend from others notifications
      [ ] Add validation btn on community validation
    [ ] View of notification by element
      [ ] Confirm push on head
  *** Mail
  [ ] Template for use case
    [X] someoneAskToBecome (Admin || Member=
    [o] Generalize someoneAskToBecome with someoneInvitingYou
    [o] someoneValidate invitation or demand to author
    [o] Follow me [Insert new follow if notif is not view]
  [ ] Insert daily mail
  [ ] Not send email if user is connected and have active view on communecter.org
  *** Preferences
  [ ] Add inscription on citizen consult that I will follow OR specific room (news + notif + mail)
  [ ] Mailing on add something in element and comment/Like/unlike on element where I am linking
  [ ] daily Report
    [ ] Add parameters (everyday [default], 2days, 3days, every week) [beginning on Monday]
    [ ] Activity Report
      [ ] New events with tags &&/|| scope I subscribe
      [ ] New organization with tags &&/|| scope I subscribe
      [ ] New projects with tags &&/|| scope I subscribe
      [ ] New item on element that I am linking
    [ ] Followers on element where I am linking
  *** News
  [ ] View of like, unlike 
  [X] Bash of object.type inbstead of object.objectType (!!!important with notification reactor!!!!)
    => ph/communecter/datamigration/ObjectObjectTypeNewsToObjectType
  *** Link
  [X] Add confirmation if someone link me to orga or projet (as event) -- ValdiateLink
  [ ] Friend relation between persons ?? (community to notify, messaging, see information ??) 
  *** Admin Communication
  [ ] Creation of news letter 
    [ ] Choose scope or theme community
    [ ] To all
    [ ] See view of newsletter 
  *** Bugs
    [X] Notification
      [X] /ADD/ Problem of url on add => id of object and not target
      [X] /FOLLOW/ Make usecase following you or target
      [X] /FOLLOW/ => no repeat on notify when I follow a user
      [X] /VALIDATE/ => undefined community
      [X] /VALIDATE/ => undefined object on check and update because go to update (add object exist in query)
      [X] BUG BUG BUG
    [X] Link
      [X] If isPendingAdmin or toBeValidated as member user is consedering as member in right access
        [X] change method Project::getContibutorsByProjectId && Organization::getMembersByOrganizationId
        [X] Add label ToBeValidated on user asking to be admin not being member yet
      [X] Menu btn => if I am member he purpose me to follow
      [X] No disconnect btn if I am member of an element
      [X] Two disconnect btn on event
    [X] Project
      [X] Project with event without address => bug on gantt indexAction
  *** Questions
    *** Event
