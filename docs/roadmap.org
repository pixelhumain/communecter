...
** @Tib 
  ...
  [ ] elements to simple dynform
    ...
    [ ] bugs
      [ ] location > tape CP > validate , ca passe 
      [ ] creating a poi
      [ ] no message on debat
      [ ] check existance click and unfocus
      [ ] create orga 
      [ ] texte/description d'une décision n'est pas mis à jour 
    [ ] switch on map , show / hide POI, point of parent
    [ ] check bad sub attributes : address.cococo
    [ ] refactor : move from ACtion to model Element
    ...
    [ ] event to organizer, name, type, allday, dates, location btn
      ...
      [ ] simplify add for sub events: préremplir les dates, lieu
...
** @Rapha et Clement
  ...
  [ ] Traduction
    [ ] Add members et tooltip
  [ ] Bugs 
    [ ] SI on change la ville du user, et qu'on clique sur repositionner alors il y une erreur
    [ ] TypeError: newsObj.target.name is undefined : Lorsqu'on va arrive sur la communauté depuis (detail , news ou gallery);
    [ ] Design : lorsqu'on veut modifier la description ou shortDescription , le panel de x-edit se cache derrier le menu de gauche.
