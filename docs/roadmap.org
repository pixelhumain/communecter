...
** @Bouboule 
  ...
  *** Notifications
  ...
  [ ] Commons method called to construct notification
    [X] /Follow/, /Ask/ to become member or admin
    [X] /Accept/ an admin or member (include new sublevel in notficationTree)
    [X] /Invite/ 
      [X] Generalize invitation as event for organization and project
    [X] /Confirm/ an invitation to join a element (for moment event)
    [X] /Join/ for creator of event, member of orga or project
  ...
  [ ] Friend relation between persons ?? (community to notify, messaging, see information ??) 
    *** Bug
    [X] If isPendingAdmin or toBeValidated as member user is consedering as member in right access
      => change method Project::getContibutorsByProjectId && Organization::getMembersByOrganizationId
      => Add label ToBeValidated on user asking to be admin not being member yet
  *** Admin Communication
  ...
  *** Bugs
    [ ] Notification
      [ ] /ADD/ Problem of url on add => id of object and not target
      [ ] /FOLLOW/ Make usecase following you or target
* ************************************************************    
