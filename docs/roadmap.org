...
** @Sylvain 
    ...
    [ ] Gérer le retour de mail en erreur 
      [X] créer un hook + erreur dans mailerror
      [ ] creation de notif pour super admin
      [ ] page admin avec les erreurs de mails
...
** @Tib 
  ...
  [ ] elements to simple dynform
    [ ] $type::afterSave : connect specific post process for each elem
      [ ] organization
      [ ] project
      [ ] event
